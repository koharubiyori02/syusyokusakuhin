...
void Fade::Init()
{
	...
	//フェードアウトの場合は初期アルファ値を0にする
	if (m_FadeName == FadeType::FadeOut)
		m_Alpha = 0.0f;
}
...
void Fade::Update()
{
	if (m_FadeName == FadeType::FadeIn)//フェードインの場合アルファ値を0に近づける
	{
		m_Alpha -= FADE_IN_SPEED;
		if (m_Alpha <= 0.0f)
		{
			m_Alpha = 0.0f;
		}
	}
	else if (m_FadeName == FadeType::FadeOut)//フェードアウトの場合アルファ値を1に近づける
	{
