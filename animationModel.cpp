...
void AnimationModel::Load( const char *FileName )
{
	...
	//テクスチャ読み込み
	for(int i = 0; i < static_cast<signed>(m_AiScene->mNumTextures); i++)
	{
...
void AnimationModel::Update(const char* AnimationName1, int Frame1, const char* AnimationName2, int Frame2,float BlendRatio)
{
	...
	for (auto pair : m_Bone)
	{
		...
		for (unsigned int d = 0; d < animation2->mNumChannels; d++)
		{
			if (animation2->mChannels[d]->mNodeName == aiString(pair.first))
			{
				nodeAnim2 = animation2->mChannels[d];
				break;
			}
		}
		...
		aiQuaternion rot;
		aiQuaternion::Interpolate(rot, rot1, rot2, BlendRatio);//救援線形補間
